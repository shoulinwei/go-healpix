...
int64_t healpix_rangesearch_radec_simple2(double ra, double dec, double radius,
                                         int Nside, int approx, int64_t *indices) {
    FILE *f1 = fopen("/opt/temp/healpix_go.log","w+");
    fprintf(f1,"ra:%f,dec:%f,radius:%f,nside:%d\n",ra, dec, radius, Nside);
    fclose(f1);
    double xyz[3];
    ll* hps = ll_new(256);
    int64_t n_pixels;
